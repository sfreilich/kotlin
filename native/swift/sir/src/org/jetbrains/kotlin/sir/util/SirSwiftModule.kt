...
object SirSwiftModule : SirModule() {
    ...
    private val utf16Extension = extension(SirNominalType(utf16, parent = SirNominalType(unicode)))

    val utf16CodeUnit = utf16Extension.addTypealias("CodeUnit", SirNominalType(utf16))
}

private fun SirMutableDeclarationContainer.struct(typeName: String) = this.addChild {
    buildStruct {
...
private fun SirMutableDeclarationContainer.extension(type: SirType) = addChild {
    buildExtension {
        origin = externallyDefined("extension ${type.swiftName}")
        visibility = SirVisibility.PUBLIC
        extendedType = type
    }
}
