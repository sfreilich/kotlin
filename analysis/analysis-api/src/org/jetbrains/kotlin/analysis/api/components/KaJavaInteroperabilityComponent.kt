...
public interface KaJavaInteroperabilityComponent {
    ...
    /**
     * Maps the given [PsiClass] declaration to the class symbol.
     *
     * Returns `null` for anonymous classes, type parameters (which are also [PsiClass]es), and for Kotlin light classes.
     */
    public val PsiClass.namedClassSymbol: KaNamedClassSymbol?

    /**
     * Maps the given [PsiMember] method or field to the callable symbol.
     *
     * Returns `null` for local declarations.
     */
     ...
     *
     * Note: [containingJvmClassName] is only applicable for common or JVM modules only, and returns `null` for non-JVM modules.
     */
    @KaExperimentalApi
