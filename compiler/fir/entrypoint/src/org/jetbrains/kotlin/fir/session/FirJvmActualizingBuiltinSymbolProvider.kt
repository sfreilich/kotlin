...
/**
 * It's initialized on the top of module-based provides for platform source sets.
 *
...
class FirJvmActualizingBuiltinSymbolProvider(
    private val dependentSourceSymbolProviders: List<FirSymbolProvider>,
    ...
    override fun getClassLikeSymbolByClassId(classId: ClassId): FirRegularClassSymbol? {
        for (symbolProvider in dependentSourceSymbolProviders) {
            ...
            // If there are multiple same declarations, they will be reported as redeclarations by a checker
    ...
    @FirSymbolProviderInternals
    private inline fun <reified T : FirCallableSymbol<*>> getTopLevelSymbolsTo(
        destination: MutableList<T>,
        crossinline getFunction: (FirSymbolProvider, MutableList<T>) -> Unit
    ) {
        val newDestination = mutableListOf<T>()
        for (symbolProvider in dependentSourceSymbolProviders) {
            newDestination.clear()
            getFunction(symbolProvider, newDestination)
            ...
            getFunction(builtinSymbolProvider, destination)
            return
        }
    }
