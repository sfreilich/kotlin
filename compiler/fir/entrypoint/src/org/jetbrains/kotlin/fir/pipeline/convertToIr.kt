...
private class Fir2IrPipeline(
    ...
    private fun Fir2IrConversionResult.createExtraExpectSyntheticDeclarationsIfStdlib(): ExpectTopLevelDeclarations? {
        ...
        for (firOutput in outputs) {
            ...
            val stdlibBuiltinSyntheticFunctionInterfaceProvider =
                dependencyProviders.filterIsInstance<FirStdlibBuiltinSyntheticFunctionInterfaceProvider>().singleOrNull() ?: continue
    ...
    private fun Fir2IrConversionResult.createIrActualizer(): IrActualizer? {
        return runIf(dependentIrFragments.isNotEmpty()) {
            val extraExpectSyntheticDeclarations: ExpectTopLevelDeclarations? = createExtraExpectSyntheticDeclarationsIfStdlib()
            IrActualizer(
                ...
                dependentIrFragments,
                listOfNotNull(
                    // extraExpectSyntheticDeclarations?.let { IrSyntheticExtraActualDeclarationExtractor(it, irBuiltIns) },
