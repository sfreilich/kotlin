...
internal class ExpectActualCollector(
    ...
    private val actualDeclarationExtractor: IrActualDeclarationExtractor?,
) {
    fun collectClassActualizationInfo(): ClassActualizationInfo {
        ...
        return ActualDeclarationsCollector.collectActualsFromFragments(
            fragmentsWithActuals, expectTopLevelDeclarations, actualDeclarationExtractor
        )
    }
...
private class ActualDeclarationsCollector(private val expectTopLevelDeclarations: ExpectTopLevelDeclarations) {
    companion object {
        fun collectActualsFromFragments(
            ...
            actualClassExtractor: IrActualDeclarationExtractor?
            ...
            if (actualClassExtractor != null) {
                collector.collectExtraActualClasses(actualClassExtractor)
            }
    ...
    private fun collectExtraActualClasses(actualDeclarationExtractor: IrActualDeclarationExtractor) {
        for (classSymbol in expectTopLevelDeclarations.classes.values) {
            collectExtraActualClasses(actualDeclarationExtractor, classSymbol.owner)
        }
        for ((callableId, callableSymbols) in expectTopLevelDeclarations.callables) {
            ...
            collectExtraActualCallables(actualDeclarationExtractor, expectTopLevelCallables, callableId)
        }
    }

    private fun collectExtraActualClasses(actualDeclarationExtractor: IrActualDeclarationExtractor, expectClass: IrClass) {
        ...
        for (declaration in expectClass.declarations) {
            if (declaration is IrClass) {
                collectExtraActualClasses(actualDeclarationExtractor, declaration)
            }
        }
    }

    private fun collectExtraActualCallables(
        actualDeclarationExtractor: IrActualDeclarationExtractor,
